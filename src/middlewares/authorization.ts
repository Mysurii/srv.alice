...
import { ApiErrorResponse } from './../utils/exceptions';
import type { TokenPayload, User } from '../models/user.model';
...
const roleSpecificEndpoints: Array<RoleSpecificEndpoint> = [{ endpoint: '/api/products', role: 'admin', methods: ['DELETE', 'PUT', 'POST'] }];
...
export default function authorizationMiddleware(request: Request, response: Response, next: NextFunction) {
  ...
  verify(accessToken, env_variables.TOKENS.secret, async (error, payload) => {
    ...
    try {
      ...
      user = await userRepository.getUserById(_id);
    } catch (error) {
      const excpetion = new ApiErrorResponse('Something went wrong with retrieving the user', HTTP_STATUS.INTERNAL_SERVER);
    ...
    for (const { endpoint, methods, role } of roleSpecificEndpoints) {
      const includesEndpoint = typeof endpoint === 'string' ? requestedEndpoint.includes(endpoint) : endpoint.test(requestedEndpoint);

      const cantEnter = includesEndpoint && role !== user.role && (methods.includes('*') || methods.includes(request.method as methods));
